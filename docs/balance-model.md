...
- The remaining Clam broad-report gap is narrower now: the first real siege-runtime pass woke up `siege_damage`, `sapper_speed`, and `siege_range`, and the utility speed/runtime pass pulled `utility_unit_speed` and `utility_train_speed` back off the negative line, so those rows are no longer misleading dead-weight reads. `siege_demolish_power`, `vision_range`, `heal_power`, and the defense wall/tower rows are the remaining flat stage-6 seams.
...
- The stage-6 Clam report is also now clearer about what remains behavioral versus missing wiring: `utility_vision_range` has a live world consumer in both explored fog and live fog rendering, but it still reads flat in the post-match stage-6 loop, so the remaining problem there is scenario expression rather than a dead runtime hook.
