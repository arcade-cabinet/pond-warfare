...
- The remaining Clam broad-report gap is narrower now: the first real siege-runtime pass woke up `siege_damage`, `sapper_speed`, and `siege_range`, the utility speed/runtime pass pulled `utility_unit_speed` and `utility_train_speed` back off the negative line, the stage-6 support-arbitration pass made `utility_heal_power` express strongly in the post-match loop, and the fog/runtime report pass finally pulled `utility_vision_range` off dead zero. The remaining flat stage-6 seams are now `siege_demolish_power` plus the defense wall/tower rows, with `vision_range` only weakly expressed rather than missing.
