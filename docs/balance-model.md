...
- The remaining Clam broad-report gap is narrower now: the first real siege-runtime pass woke up `siege_damage`, `sapper_speed`, and `siege_range`, the utility speed/runtime pass pulled `utility_unit_speed` and `utility_train_speed` back off the negative line, the stage-6 support-arbitration pass made `utility_heal_power` express strongly in the post-match loop, the fog/runtime report pass pulled `utility_vision_range` off dead zero, and the new lodge-repair savings guard made `defense_wall_hp_t0` a clearly positive row instead of a dead one. The remaining flat stage-6 seams are now `defense_tower_damage_t0`, `economy_gather_radius_t0`, and `siege_demolish_power_t0`, with `vision_range` only weakly expressed rather than missing.
