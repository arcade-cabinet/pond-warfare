...
- The remaining Clam broad-report gap is narrower now: the first real siege-runtime pass woke up `siege_damage`, `sapper_speed`, and `siege_range`, so those rows are no longer dead config. `siege_demolish_power`, `vision_range`, `heal_power`, and the defense wall/tower rows are the remaining flat stage-6 seams.
