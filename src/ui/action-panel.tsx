...
function ActionButton({ def }: { def: ActionButtonDef; index: number }) {
  ...
  return (
    ...
    >
      ...
      <span class="font-numbers font-normal text-[8px] md:text-[10px] mt-1 whitespace-pre-line" style={{ color: 'var(--pw-accent)' }}>
        {def.cost}
      </span>
    </button>
  );
      ...
      <div class="flex gap-2 flex-wrap">
        {items.map((item, i) => (
          <button
            type="button"
            key={`q-${i}`}
            class="relative w-8 h-8 min-w-[44px] min-h-[44px] rounded cursor-pointer overflow-hidden transition-colors"
            style={{
...
function TabButton({
  ...
  return (
    <button
      ...
      style={{
        background: active ? 'var(--pw-bg-elevated)' : 'var(--pw-bg-surface)',
...
export function ActionPanel() {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--pw-bg-surface)' }}
    >
      {/* Tab bar */}
      {buttons.length > 0 && (
        <div class="flex flex-shrink-0" style={{ borderBottom: '1px solid var(--pw-border)' }}>
          <TabButton
            tab="train"
