...
describe('achievements', () => {
  function requireAchievement(id: string) {
    const achievement = ACHIEVEMENTS.find((a) => a.id === id);
    expect(achievement).toBeDefined();
    if (!achievement) {
      throw new Error(`Expected ${id} achievement to exist`);
    }
    return achievement;
  }
  ...
  it('Weather Master requires win + all 4 weather types', () => {
    const ach = requireAchievement('weather_master');
  ...
  it('Market Mogul requires 10 trades', () => {
    const ach = requireAchievement('market_mogul');
  ...
  it('Co-op Victory requires win in co-op mode', () => {
    const ach = requireAchievement('coop_victory');
  ...
  it('Daily Dedication requires 7 daily challenges', () => {
    const ach = requireAchievement('daily_dedication');
  ...
  it('Level 10 requires player level 10', () => {
    const ach = requireAchievement('level_10');
  ...
  it('Puzzle Pro requires 10 perfect puzzles', () => {
    const ach = requireAchievement('puzzle_pro');
  ...
  it('Event Survivor requires 8 random events', () => {
    const ach = requireAchievement('event_survivor');
  ...
  it('Wall Builder requires 10 wall segments', () => {
    const ach = requireAchievement('wall_builder');
